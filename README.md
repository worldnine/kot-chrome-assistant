...
各操作（出勤 / 退勤 / 休憩開始 / 休憩終了 / 打刻状況)は App Intents として定義されており、
ショートカット.app のアクションとして使えます。`shortcuts run` はライブラリ内の
ショートカットを実行するコマンドなので、最初に一度だけラッパーを作成します:

1. ショートカット.app で「＋」（新規ショートカット）
2. 右側のアクション検索で「KOTAssistant」を検索し、「打刻状況」アクションを追加
3. ショートカット名を「打刻状況」にして閉じる
4. 必要に応じて「出勤」「退勤」「休憩開始」「休憩終了」も同様に作成
...
App Shortcuts（自動登録）も提供しているため、Spotlight や Siri からは「KOTAssistantで出勤」のように呼び出せます
（システムのインデックス反映に時間がかかる場合があります。アプリは /Applications に置いてください）。
